        ...
        <Modal :title="modalTitle" v-model="userModalVisible" :mask-closable='false' :width="500">
            <Form ref="userForm" :model="userForm" :label-width="70" :rules="userFormValidate">
                ...
                <FormItem label="手机号" prop="mobile">
                    <Input v-model="userForm.mobile" :maxlength="11"/>
                </FormItem>
...
import { allBackRequestUrl } from "@/utils/httpRequestUrl";
...
export default {
  ...
  methods: {
    init() {
      console.log(allBackRequestUrl.deleteUserByIdUrl);
      this.getUserList();
    },
    changePage(v) {
    ...
    getUserList() {
      ...
      // var mockData = [{address:null,avatar:"https://s1.ax1x.com/2018/05/19/CcdVQP.png",createBy:null,
      //           createTime:"2018-06-29 17:27:54",delFlag:0,description:null,email:null,id:"25053975845801984",
      //           mobile:"13322207892",nickName:null,password:null,permissions:null,
      //           roles:[{createBy:null,createTime:null,defaultRole:null,delFlag:0,id:"496138616573953",name:"ROLE_USER",permissions:null,updateBy:null,updateTime:null}],
      //           sex:null,status:0,type:0,updateBy:null,updateTime:"2018-06-29 17:27:54",username:"adminadmin"}];
      // this.loading = false;
      // this.data = mockData;
      // this.total = 0;
      ...
      // this.getRequest("http://192.168.1.104:8080/user/list").then(res => {  
      
      // this.getRequest("/user/list", this.searchForm).then(res => {
      this.getRequest("/user/list", this.searchForm).then(res => {  
        this.loading = false;
        if (res.success === true) {
        ...
        if (valid) {
          ...
          if (this.modalType === 1) {
            ...
            url = "/user/update";
                ...
                this.putRequest(url, this.userForm).then(res => {
          ...
          // }
          this.submitLoading = true;
          var createUser = this.userForm;
          delete createUser.id;
          createUser.status = 0;
          createUser.type = 0;
          // this.postRequest(url, this.userForm).then(res => {
          this.postRequest(url, createUser).then(res => {
            this.submitLoading = false;
            if (res.success === true) {
    ...
    enable(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.postRequest("/user/enable/" + v.id).then(res => {
            if (res.success === true) {
              this.$Message.success("操作成功");
              this.init();
            }
    ...
    disable(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.postRequest("/user/disable/" + v.id).then(res => {
            if (res.success === true) {
              this.$Message.success("操作成功");
              this.init();
            }
    ...
    remove(v) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.deleteRequest("/user/delete", { ids: v.id }).then(res => {
            if (res.success === true) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          this.deleteRequest("/user/delete", { ids: ids }).then(res => {
            if (res.success === true) {
              this.$Message.success("删除成功");
              this.init();
            }
